...
//|        (BTC-COREからの派生 / XAUUSD(GOLD)専用チューニング)     |
//+------------------------------------------------------------------+
#property copyright "GOLD-CORE"
...
input long   MagicNumberLong  = 20260301;     // ロング用マジックナンバー(BTC-COREと並走干渉回避)
input long   MagicNumberShort = 20260302;     // ショート用マジックナンバー(BTC-COREと並走干渉回避)
input string EAComment        = "GOLD-CORE";  // コメント
...
input double MinDistancePips = 50.0;     // 距離の下限(pips) ※BTC:5 → GOLD:50
input double MaxDistancePips = 800.0;    // 距離の上限(pips) ※BTC:2000 → GOLD:800
...
//=== 曜日フィルター ===
input group "===== 曜日フィルター(GOLD平日稼働 / 土日は市場クローズ) ====="
input bool 月曜日稼働 = true;    // 月曜日 取引ON/OFF
input bool 火曜日稼働 = true;    // 火曜日 取引ON/OFF
input bool 水曜日稼働 = true;    // 水曜日 取引ON/OFF
input bool 木曜日稼働 = true;    // 木曜日 取引ON/OFF
input bool 金曜日稼働 = true;    // 金曜日 取引ON/OFF
input bool 土曜日稼働 = false;   // 土曜日 取引ON/OFF
input bool 日曜日稼働 = false;   // 日曜日 取引ON/OFF

//=== 時間フィルター(日本時間) ===
input group "===== 時間フィルター(日本時間) ====="
// ※ XM・HFMの夏時間/冬時間は自動切替されます(手動設定不要)
// ※ 0:00〜24:00の範囲で設定してください(24:00は翌0:00扱い)
input bool UseTimeFilter      = false;   // 時間フィルター使用 ※GOLD:まず無制限でBT→後で最適時間帯を絞る
...
//| グローバル変数                                                    |
//+------------------------------------------------------------------+
double   g_PipsToPrice;           // 1pipsを価格に変換する係数(XAUUSD/GOLD: 1pip=0.01=SYMBOL_POINT)
...
int OnInit()
{
   ...
   {
      Print("認証失敗 - EAを停止します");
      Alert("GOLD-CORE: 認証に失敗しました。口座番号をご確認ください。");
      return INIT_FAILED;
   }
   ...
   // pips換算係数(XAUUSD/GOLD専用: digits=2 / 1pip = 0.01 = SYMBOL_POINT)
   int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   g_PipsToPrice = SymbolInfoDouble(_Symbol, SYMBOL_POINT);  // = 0.01
   ...
   Print("GOLD-CORE v1.00 起動 (XAUUSD/GOLD専用 / 自動モード固定)");
