...
{
   MODE_NORMAL  = 0,   // 通常モード(EURUSD等のFX)
   MODE_BTCUSD  = 1    // BTCUSDモード(1pip=1ドル換算)
};

input group "===== 銘柄モード ====="
input ENUM_SYMBOL_MODE SymbolMode = MODE_NORMAL;   // 銘柄モード

//=== 手動エントリーモード ===
input group "===== 手動エントリーモード ====="
input bool   ManualEntryMode  = false;        // true=手動モード / false=自動モード
input double ManualLot        = 0.01;         // 手動エントリー初期ロット
...
//| グローバル変数                                                    |
//+------------------------------------------------------------------+
double   g_PointMultiplier;       // 5桁ブローカー対応係数
double   g_PipsToPrice;           // 1pipsを価格に変換する係数
...
// パネル関連
string   g_PanelPrefix = "BC_Panel_";  // オブジェクト名プレフィックス
int      g_PanelX      = 20;            // パネル左上X座標
int      g_PanelY      = 50;            // パネル左上Y座標

// ドラッグ状態管理
bool     g_IsDragging  = false;         // ドラッグ中フラグ
int      g_DragOffsetX = 0;             // マウスとパネル左上の差分X
int      g_DragOffsetY = 0;             // マウスとパネル左上の差分Y
int      g_GrabBarH    = 22;            // ドラッグバーの高さ
...
int OnInit()
{
   ...
   // pips換算係数の計算(銘柄モードに応じて切替)
   int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   string symModeName = "";

   if(SymbolMode == MODE_BTCUSD)
   {
      // BTCUSDモード: 1pip = 1ドル
      g_PointMultiplier = 0;  // 不使用
      g_PipsToPrice     = 1.0;
      symModeName       = "BTCUSDモード";
   }
   else
   {
      // 通常モード(FX): 5桁/3桁ブローカー対応
      if(digits == 5 || digits == 3)
         g_PointMultiplier = 10.0;
      else
         g_PointMultiplier = 1.0;
      g_PipsToPrice = SymbolInfoDouble(_Symbol, SYMBOL_POINT) * g_PointMultiplier;
      symModeName   = "通常モード";
   }
   ...
   Print("BTC-CORE v1.00 起動");
   Print("通貨ペア: ", _Symbol, " / Digits: ", digits, " / PipsToPrice: ", g_PipsToPrice, " [", symModeName, "]");
   ...
   if(ManualEntryMode)
   {
      ChartSetInteger(0, CHART_EVENT_MOUSE_MOVE, true); // マウス移動イベント有効化
      CreatePanel();
      Print("手動エントリーモード ON / 初期ロット:", ManualLot);
   }
...
void OnTick()
{
   ...
   // 新規エントリー(時間・曜日チェック) ※自動モード時のみ
   if(!ManualEntryMode && IsTradeTimeAllowed() && IsTradeDayAllowed())
   {
      ProcessNewEntry();
   }
...
double GetLotForStage(int stage)
{
   ...
   double baseLot = ManualEntryMode ? ManualLot : StartLot;
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
      ...
      if(leftDown)
      {
         if(!g_IsDragging)
         {
            ...
            if(mouseX >= barX && mouseX <= barX + barW &&
               mouseY >= barY && mouseY <= barY + g_GrabBarH)
            {
               g_IsDragging  = true;
   ...
   if(sparam == g_PanelPrefix + "BtnLong")
   {
      int stages = CountPositions(POSITION_TYPE_BUY);
      if(MaxStages > 0 && stages >= MaxStages)
      {
         Print("手動LONG: 最大段数に達しています");
      }
      else if(stages == 0)
      {
         OpenPosition(POSITION_TYPE_BUY, 1);
         Print("手動LONG: 1段目エントリー Lot:", ManualLot);
      }
      else
      {
         OpenPosition(POSITION_TYPE_BUY, stages + 1);
         Print("手動LONG: ", stages+1, "段目ナンピン");
      }
      ObjectSetInteger(0, g_PanelPrefix + "BtnLong", OBJPROP_STATE, false);
      ChartRedraw();
   }

   if(sparam == g_PanelPrefix + "BtnShort")
   {
      int stages = CountPositions(POSITION_TYPE_SELL);
      if(MaxStages > 0 && stages >= MaxStages)
      {
         Print("手動SHORT: 最大段数に達しています");
      }
      else if(stages == 0)
      {
         OpenPosition(POSITION_TYPE_SELL, 1);
         Print("手動SHORT: 1段目エントリー Lot:", ManualLot);
      }
      else
      {
         OpenPosition(POSITION_TYPE_SELL, stages + 1);
         Print("手動SHORT: ", stages+1, "段目ナンピン");
      }
      ObjectSetInteger(0, g_PanelPrefix + "BtnShort", OBJPROP_STATE, false);
      ChartRedraw();
   }

   if(sparam == g_PanelPrefix + "BtnCloseLong")
   {
      CloseAllPositions(POSITION_TYPE_BUY);
      Print("手動: LONGポジション全決済");
      ObjectSetInteger(0, g_PanelPrefix + "BtnCloseLong", OBJPROP_STATE, false);
      ChartRedraw();
   }

   if(sparam == g_PanelPrefix + "BtnCloseShort")
   {
      CloseAllPositions(POSITION_TYPE_SELL);
      Print("手動: SHORTポジション全決済");
      ObjectSetInteger(0, g_PanelPrefix + "BtnCloseShort", OBJPROP_STATE, false);
      ChartRedraw();
   }
}
...
//| パネル全オブジェクトを相対移動                                    |
//+------------------------------------------------------------------+
void MovePanelObjects(int dx, int dy)
{
   for(int i = ObjectsTotal(0) - 1; i >= 0; i--)
   {
      ...
      {
         int cx = (int)ObjectGetInteger(0, name, OBJPROP_XDISTANCE);
         int cy = (int)ObjectGetInteger(0, name, OBJPROP_YDISTANCE);
         ObjectSetInteger(0, name, OBJPROP_XDISTANCE, cx + dx);
         ObjectSetInteger(0, name, OBJPROP_YDISTANCE, cy + dy);
      }
   }
}
...
void CreatePanel()
{
   ...
   CreateRect(g_PanelPrefix + "BG", x-8, y-8, w+16, bh*4+gap*5+50, clrDimGray, 200);

   CreateRect(g_PanelPrefix + "DragBar", x-8, y-8, w+16, g_GrabBarH, C'50,50,80', 230);
   CreateLabel(g_PanelPrefix + "DragHint", x + w/2, y - 2, "≡ BTC-CORE [手動] ≡", 8, clrLightSteelBlue, ANCHOR_CENTER);

   CreateButton(g_PanelPrefix + "BtnLong",
                x, y + 22,
                w, bh,
                "▲ LONG エントリー",
                clrDarkGreen, clrWhite, 9);

   CreateLabel(g_PanelPrefix + "InfoLong", x + w/2, y + 22 + bh + 2, "LONG: 0段", 8, clrLightGreen, ANCHOR_CENTER);

   CreateButton(g_PanelPrefix + "BtnShort",
                x, y + 22 + bh + gap + 16,
                w, bh,
                "▼ SHORT エントリー",
                clrDarkRed, clrWhite, 9);

   CreateLabel(g_PanelPrefix + "InfoShort", x + w/2, y + 22 + bh*2 + gap + 18, "SHORT: 0段", 8, clrLightCoral, ANCHOR_CENTER);

   CreateButton(g_PanelPrefix + "BtnCloseLong",
                x, y + 22 + bh*2 + gap*2 + 32,
                (w-gap)/2, bh-8,
                "L決済",
                C'40,80,40', clrWhite, 8);

   CreateButton(g_PanelPrefix + "BtnCloseShort",
                x + (w-gap)/2 + gap, y + 22 + bh*2 + gap*2 + 32,
                (w-gap)/2, bh-8,
                "S決済",
                C'80,40,40', clrWhite, 8);
...
{
   int longStages = CountPositions(POSITION_TYPE_BUY);
   string longInfo = "LONG: " + IntegerToString(longStages) + "段";
   if(longStages > 0)
   {
      double longAvg = CalcWeightedAverage(POSITION_TYPE_BUY);
      double longPL  = CalcTotalProfit(POSITION_TYPE_BUY);
      longInfo += "  avg:" + DoubleToString(longAvg, _Digits) + "  PL:" + DoubleToString(longPL, 0);
   }
   ObjectSetString(0, g_PanelPrefix + "InfoLong", OBJPROP_TEXT, longInfo);

   int shortStages = CountPositions(POSITION_TYPE_SELL);
   string shortInfo = "SHORT: " + IntegerToString(shortStages) + "段";
   if(shortStages > 0)
   {
      double shortAvg = CalcWeightedAverage(POSITION_TYPE_SELL);
      double shortPL  = CalcTotalProfit(POSITION_TYPE_SELL);
      shortInfo += "  avg:" + DoubleToString(shortAvg, _Digits) + "  PL:" + DoubleToString(shortPL, 0);
   }
   ObjectSetString(0, g_PanelPrefix + "InfoShort", OBJPROP_TEXT, shortInfo);
...
void CreateButton(string name, int x, int y, int w, int h,
                  string text, color bgColor, color txtColor, int fontSize)
{
...
void CreateLabel(string name, int x, int y, string text,
                 int fontSize, color txtColor, ENUM_ANCHOR_POINT anchor)
{
